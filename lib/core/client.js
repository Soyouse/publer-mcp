 ...
 *
 * ⚠️ Publer n'a PAS de SDK officiel → fetch natif (Node ≥22). Le throttle (50 req/min) est délégué à
 *    p-throttle (sindresorhus, ESM, activement maintenu — pile notre cas « N appels/intervalle »).
 *    NE PAS réinventer la file/limiteur à la main. (bottleneck écarté : mort depuis 2019, CJS.)
 * ⚠️ Rate-limit Publer = PAR-COMPTE (api_key) → 1 throttle p-throttle PAR api_key (Map), jamais partagé.
...
/** Fonction throttlée (p-throttle) pour un compte (api_key) : 50 appels / 60 s, en file (zéro perte).
 *  Wrappe un dispatcher générique → tous les appels d'un même compte partagent la même fenêtre. */
function getThrottled(apiKey) {
  let t = throttledByKey.get(apiKey);
  if (!t) {
    const throttle = pThrottle({ limit: 50, interval: 60_000 });
